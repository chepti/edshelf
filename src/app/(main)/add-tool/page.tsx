...
export default function AddToolPage() {
  ...
  const onSubmit: SubmitHandler<ToolFormValues> = async (data) => {
    if (!user) {
      setSubmitError("You must be logged in to add a tool."); // Should be handled by middleware ideally
      return;
    }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-3xl font-bold mb-6">Add a New AI Tool</h1>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div>
          ...
          <Input id="tags" {...register('tags')} placeholder="e.g., productivity, writing, ai-art" />
          {/* No specific error for tags unless made required or more complex validation */}
        </div>
